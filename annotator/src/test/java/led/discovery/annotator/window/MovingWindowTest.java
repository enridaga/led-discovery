...
public class MovingWindowTest {
	...
	public void collected() {
		...
		Assert.assertTrue(mw.collected().size() == 1);
		log.info("ADD S6");
		mw.move(TestUtil.S6);
		Assert.assertTrue(mw.collected().size() == 2);
	}
	...
	public void testMoving() {
		...
		Assert.assertTrue(mw2.generatedCount == 9);
		Assert.assertTrue(mw2.collected().size() == 4);
		//
		...
		for (CoreMap s : text)
			mw2.move(s);
		Assert.assertTrue(mw2.generatedCount == expected);
