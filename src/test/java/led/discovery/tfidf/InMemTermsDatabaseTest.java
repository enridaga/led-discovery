...
public class InMemTermsDatabaseTest {
	...
	public void getTermsSize() throws IOException {
		...
		Assert.assertTrue("getTermsSize", d.countTerms() == 5);
		Assert.assertTrue("getTermId exists", d.getTermId("a") == 0);
		Assert.assertTrue("getTermId not exists", d.getTermId("aaaa") == -1);
	...
	public void getTermFrequency() throws IOException {
		d.addDocument("doc1", new String[] { "a", "b", "c", "d", "e" });
		d.addDocument("doc2", new String[] { "a", "a", "a", "d", "e" });
		d.addDocument("doc3", new String[] { "f", "g", "h", "i", "j", "k" });
	...
	public void containsTerm() throws IOException {
		d.addDocument("doc1", new String[] { "a", "b", "c", "d", "e" });
		Assert.assertTrue(d.containsTerm("a"));
		Assert.assertTrue(d.containsTerm("b"));
		Assert.assertTrue(d.containsTerm("c"));
		Assert.assertTrue(d.containsTerm("d"));
		Assert.assertTrue(d.containsTerm("e"));
		Assert.assertTrue(!d.containsTerm("f"));
	}
